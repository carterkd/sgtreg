...
version 12.0
	...
	tempvar x s l m sigmid
	qui gen `sigmid' = `sigma' / (`q'^(1/`p')*sqrt((3*`lambda'^2+1)*(exp(lngamma(3/`p') + lngamma(`q'-2/`p') - lngamma(`q' + 1/`p'))/exp(lngamma(1/`p') + lngamma(`q') - lngamma(`q' + 1/`p')))-4*`lambda'^2*(exp(lngamma(2/`p') + lngamma(`q'-1/`p') - lngamma(`q' + 1/`p'))/exp(lngamma(1/`p') + lngamma(`q') - lngamma(`q' + 1/`p')))^2))
